...
function parseCSV(csvText) {
    ...
    return lines.map(line => {
        ...
        return headers.reduce((obj, nextKey, index) => {
            obj[nextKey.trim()] = data[index].trim();  // Asegurarse de limpiar espacios en blanco
            return obj;
        }, {});
    });
}
...
function mostrarImagenes(data) {
    ...
    data.forEach(imagen => {
        ...
        // Construir la URL de la imagen según el ID
        const imageURL = `https://filedn.com/lRAMUKU4tN3HUnQqI5npg4H/Plantix/Imagenes/imagen_${imagen.ID}.png`;
        imgDiv.innerHTML = `
            <img src="${imageURL}" alt="Diagnóstico: ${imagen.diagnostico}" class="image">
